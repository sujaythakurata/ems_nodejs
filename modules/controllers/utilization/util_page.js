...
exports.loadinfo = (req, res)=>{
	...
	let query = `
	SET SQL_MODE='';
	SELECT M_ID, M_NAME FROM METER_LIST WHERE C_ID = '${req.client_id}';
	SELECT AVG_TABLE.M_NAME, AVG_TABLE.M, AVG_TABLE.C_ID, ROUND(COUNT(I)/3600, 3) AS FULL_LOAD_HOUR,COUNT(I) AS FULL_LOAD_SECOND,  AVG_TABLE.INSERTDATETIME FROM 
	...
	INSERTDATETIME>=CONCAT(DATE_SUB(CURRENT_DATE, INTERVAL 7 DAY), " ", "00:00:00") AND 
	...
	AND AVG_TABLE.I>=CONFIG.FULL_LOAD_I GROUP BY AVG_TABLE.M, AVG_TABLE.C_ID ORDER BY AVG_TABLE.M_NAME;

	SELECT AVG_TABLE.M_NAME, AVG_TABLE.M, AVG_TABLE.C_ID, ROUND(COUNT(I)/3600, 3) AS NO_LOAD_HOUR,COUNT(I) AS NO_LOAD_SECOND,  AVG_TABLE.INSERTDATETIME FROM 
	(SELECT METERS.M_NAME, ROUND((I1+I2+I3)/3, 2) AS I,I1, I2, I3,  M , EMS_T1.C_ID, INSERTDATETIME  FROM EMS_T1, 
	(SELECT * FROM METER_LIST WHERE C_ID = '${req.client_id}')METERS WHERE EMS_T1.M = METERS.M_ID AND EMS_T1.C_ID = METERS.C_ID AND 
	INSERTDATETIME>=CONCAT(DATE_SUB(CURRENT_DATE, INTERVAL 7 DAY), " ", "00:00:00") AND 
	INSERTDATETIME<=CONCAT(CURRENT_DATE, " ", "23:59:59"))AVG_TABLE, 
	(SELECT * FROM METER_CONFIG)CONFIG 
	WHERE CONFIG.M_ID = AVG_TABLE.M AND CONFIG.C_ID  = AVG_TABLE.C_ID 
	AND AVG_TABLE.I>=CONFIG.NO_LOAD_I AND AVG_TABLE.I<CONFIG.FULL_LOAD_I GROUP BY AVG_TABLE.M, AVG_TABLE.C_ID ORDER BY AVG_TABLE.M_NAME;

	SELECT AVG_TABLE.M_NAME, AVG_TABLE.M, AVG_TABLE.C_ID, ROUND(COUNT(I)/3600, 3) AS OFF_LOAD_HOUR,COUNT(I) AS OFF_LOAD_SECOND,  AVG_TABLE.INSERTDATETIME FROM 
	(SELECT METERS.M_NAME, ROUND((I1+I2+I3)/3, 2) AS I,I1, I2, I3,  M , EMS_T1.C_ID, INSERTDATETIME  FROM EMS_T1, 
	(SELECT * FROM METER_LIST WHERE C_ID = '${req.client_id}')METERS WHERE EMS_T1.M = METERS.M_ID AND EMS_T1.C_ID = METERS.C_ID AND 
	INSERTDATETIME>=CONCAT(DATE_SUB(CURRENT_DATE, INTERVAL 7 DAY), " ", "00:00:00") AND 
	INSERTDATETIME<=CONCAT(CURRENT_DATE, " ", "23:59:59"))AVG_TABLE, 
	(SELECT * FROM METER_CONFIG)CONFIG 
	WHERE CONFIG.M_ID = AVG_TABLE.M AND CONFIG.C_ID  = AVG_TABLE.C_ID 
	AND AVG_TABLE.I<=CONFIG.OFF_I GROUP BY AVG_TABLE.M, AVG_TABLE.C_ID ORDER BY AVG_TABLE.M_NAME;
	`;
	global.pool.getConnection((err, conn)=>{
		if(err){
			global.logerr.log(err);
