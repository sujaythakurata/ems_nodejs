...
function USERS(pool) {
	...
	let query = `CREATE TABLE IF NOT EXISTS USERS(
		...
		CREATE INDEX IF NOT EXISTS USER_SEARCH ON USERS(USER_EMAIL, CLIENT_ID);
	`;
...
function METER_LIST(pool){
		...
		CREATE INDEX IF NOT EXISTS METER_LIST_SEARCH ON METER_LIST(M_ID, C_ID, CREATED_TIME);
	`;
...
function METER_CONFIG(pool) {
	let query = `CREATE TABLE IF NOT EXISTS METER_CONFIG(
		...
		CREATE INDEX IF NOT EXISTS METER_CONFIG_SEARCH ON METER_CONFIG(M_ID, C_ID);
	`;
...
function EMS_T1(pool) {
	let query = `CREATE TABLE IF NOT EXISTS EMS_T1(
		...
		CREATE INDEX IF NOT EXISTS EMS_T1_SEARCH ON EMS_T1(M, C_ID);
		CREATE INDEX IF NOT EXISTS EMS_T1_SEARCH_BY_TIME ON EMS_T1(INSERTDATETIME);
	`;
...
function EMS_T2(pool) {
	let query = `CREATE TABLE IF NOT EXISTS EMS_T2(
		...
		CREATE INDEX IF NOT EXISTS EMS_T2_SEARCH ON EMS_T2(M, C_ID);
		CREATE INDEX IF NOT EXISTS EMS_T2_SEARCH_BY_TIME ON EMS_T2(INSERTDATETIME);
	`;
...
function KWH_DAILY(pool){
	...
	CREATED_DATE DATE,
	UPDATE_TIME TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP);
	CREATE INDEX IF NOT EXISTS KWH_DAILY_SEARCH ON KWH_DAILY(M, C_ID, CREATED_DATE, UPDATE_TIME);
	`;
...
function KWH_MONTHLY(pool){
	...
	YEAR INT(4),
	UPDATE_TIME TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP);
	CREATE INDEX IF NOT EXISTS KWH_MONTHLY_SEARCH ON KWH_MONTHLY(M, C_ID, MONTH, YEAR, UPDATE_TIME);
	`;
...
function KWH_YEARLY(pool){
	...
	YEAR INT(4),
	UPDATE_TIME TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP);
	CREATE INDEX IF NOT EXISTS KWH_YEARLY_SEARCH ON KWH_YEARLY(M, C_ID, UPDATE_TIME, YEAR);
	`;
